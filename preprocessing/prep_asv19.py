...
def prepare_dataset(data_root, input_tsv_path, output_csv):
    """
    Prepare label file for the new dataset.
    ...
    input_tsv_path_obj = Path(input_tsv_path).resolve()

    # The file path is correct, but we'll check for both .tsv and .txt extensions
    ...
    print(f"Scanning for label file: {input_tsv_path_obj}")
    
    all_records = []

    try:
        # Read the TSV or TXT file, specifying no header and using a space as the delimiter
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing TSV entries"):
            # The new format has filename in column 1 and label in column 4
        ...
        print(f"Warning: Could not process file {input_tsv_path_obj}. Error: {e}")
        return
